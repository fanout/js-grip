...
// WebSocketContext
var WebSocketContext = defineClass(function(id, meta, in_events, prefix) {
    this.id = id;
    this.inEvents = in_events;
    this.readIndex = 0;
    this.accepted = false;
    ...
    recv: function() {
        ...
        if (e.type == 'TEXT' || e.type == 'BINARY') {
            ...
            if (e.content !== undefined && e.content.length == 2) {
                this.close_code = jspack.Unpack('H', e.content)[0];
            }
            return null;
        } else {
    ...
    send: function(message) {
        this.outEvents.push(new WebSocketEvent('TEXT', 'm:' + message));
    },
    sendControl: function(message) {
        this.outEvents.push(new WebSocketEvent('TEXT', 'c:' + message));
    },
    subscribe: function(channel) {
        this.sendControl(webSocketControlMessage(this.prefix + 'subscribe',
                {'channel': channel}));
    },
    unsubscribe: function(channel) {
        this.sendControl(webSocketControlMessage(this.prefix + 'unsubscribe',
                {'channel': channel}));
    },
    detach: function(channel) {
        this.sendControl(webSocketControlMessage('detach'));
    }
});
